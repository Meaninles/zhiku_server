...
func (e *FileUploadAndDownloadService) UploadFileWithDescription(header *multipart.FileHeader, noSave string, description string, kbId string) (file example.ExaFileUploadAndDownload, err error) {
	oss := upload.NewOss()
	filePath, key, uploadErr := oss.UploadFileWithKBID(header, kbId)
	if uploadErr != nil {
		panic(err)
	}
	if noSave == "0" {
		s := strings.Split(header.Filename, ".")
		fmt.Println("chatgpt key: " + key + "   path: " + filePath)
		f := example.ExaFileUploadAndDownload{
			Url:         filePath,
			Name:        header.Filename,
			Tag:         s[len(s)-1],
